...
import { getImageUrl } from '@/types/home';
...
export default function MyBookingsPage() {
    ...
    const handleImageError = (bookingId: string, imageUrl: string) => {
        console.error('❌ Erreur chargement image booking:', bookingId);
        console.error('URL:', imageUrl);
        setImageErrors(prev => ({ ...prev, [bookingId]: true }));
    };

    const handleImageLoad = (bookingId: string) => {
        console.log('✅ Image chargée booking:', bookingId);
        setImageErrors(prev => ({ ...prev, [bookingId]: false }));
    };
                ...
                {bookings.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {bookings.map((booking) => {
                            const sortedImages = booking.home.home_image && booking.home.home_image.length > 0
                                ? [...booking.home.home_image].sort((a, b) => a.ordernum - b.ordernum)
                                : [];

                            const firstImageKey = sortedImages.length > 0 ? sortedImages[0].imagekey : null;
                                ...
                                >
                                    <div className="relative">
                                        <div className="w-full h-48 bg-gray-200">
                                            {imageUrl && !imageErrors[booking.idbooking] ? (
                                                <img
                                                    key={`booking-${booking.idbooking}-${imageUrl}`}
                                                    src={imageUrl}
                                                    alt={booking.home.namehome}
                                                    className="w-full h-full object-cover"
                                                    onError={() => handleImageError(booking.idbooking, imageUrl)}
                                                    onLoad={() => handleImageLoad(booking.idbooking)}
                                                />
                                            ) : (
